...
(defun dbpe (&key (stream t) (increase-count? t) (place-for-count 4) (trailing-newline? t) (delimiter " ") (add-prefixes? t) output-list)
  "extended"
  (let* ((prefix (format nil (format nil "~~~AA " place-for-count) (format nil "~A>" *dbp-count*)))
         (prefix-chars (coerce prefix 'list)))
    (labels ((%apply-autistic-formatting (list)
             ...
             (%intercept-newlines-with-prefix (string)
               (coerce (loop for c across string appending (if (eq c #\newline) (cons #\newline prefix-chars) (list c))) 'string)))
